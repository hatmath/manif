package com.projet_integrateur.app.controller.http;
...
import com.projet_integrateur.app.controller.http.internal.StreamRequestBody;
...
public class HttpClient
{
    ...
    public static HttpClient getInstance()    {  if (mInstance == null) { synchronized (HttpClient.class) {  if (mInstance == null) mInstance = new HttpClient();   }    }   return mInstance;    }

    private HttpClient() { mOkHttpClient = new OkHttpClient();   mUIHandler = new Handler(Looper.getMainLooper());  }

    public OkHttpClient getOkHttpClient()   {    return mOkHttpClient;  }
    private Handler getUIHandler()          {   return mUIHandler;  }
    ...
    {
        if (callback == null)    callback = sDefaultCallback;

        callback.onRequestStart();
        Call call = getInstance().getOkHttpClient().newCall(request);
        try    {  Response response = call.execute();    callback.onResponse(response);   }
        catch (IOException e) {  e.printStackTrace();    callback.onFailure(null, e);   }
    }

    public static void asyncRequest(Request request, BaseCallback callback)
    {
        if (callback == null)   callback = sDefaultCallback;
        callback.setUIHandler(getInstance().getUIHandler());    callback.onRequestStart();
        getInstance().getOkHttpClient().newCall(request).enqueue(callback);
    }

    public static void getSync(String url, final BaseCallback callback) { getSync(url, null, callback);    }
    public static void getSync(String url, RequestParams requestParams, final BaseCallback callback)
    {
        final Request request = new Request.Builder().url(getUrlWithQueryString(url, requestParams)).build();
    ...
    {
        if (requestParams == null || requestParams.getUrlParams().isEmpty())  return url;

        if (url == null) throw new IllegalArgumentException("url == null");
        String paramUrl = requestParams.toString();

        if (url.indexOf('?') == -1)      url += "?" + paramUrl;
        else                             url += "&" + paramUrl;
        return url;
    }


    public static void postSync(String url, final BaseCallback callback)   {   postSync(url, null, callback);  }
    public static void postSync(String url, RequestParams requestParams, final BaseCallback callback)
    {
        Request request =  new Request.Builder().url(url).post(createPostRequest(requestParams)).build();
        syncRequest(request, callback);
    }

    public static void post(String url, final BaseCallback callback) {   post(url, null, callback);   }
    ...
    public static RequestBody createPostRequest(RequestParams requestParams)
    {
        ...
        if (requestParams == null || (requestParams.getFileParams().isEmpty() && requestParams.getStreamParams().isEmpty()))
             requestBody = buildFormRequestBody(requestParams);
        else requestBody = buildMultipartRequestBody(requestParams);
        return requestBody;
    }
    ...
    private static RequestBody buildMultipartRequestBody(@Nullable RequestParams requestParams)
    {
        ...
        if (requestParams != null)
        {
            ...
            for (ConcurrentHashMap.Entry<String, RequestParams.FileWrapper> entry : fileParams.entrySet())
            {
                ...
                RequestBody fileBody = RequestBody.create(MediaType.parse(fileWrapper.contentType),  fileWrapper.file);
            ...
            for (ConcurrentHashMap.Entry<String, RequestParams.StreamWrapper> entry : streamParams.entrySet())
            {
                ...
                RequestBody streamBody = StreamRequestBody.create(MediaType.parse(streamWrapper.contentType), streamWrapper.inputStream);
